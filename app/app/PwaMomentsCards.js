...
export default function PwaMomentsCards({profile,onProfileChange}){
 ...
 return <>
  ...
  {tutorial&&<div className="install-overlay" role="dialog" aria-modal="true" aria-labelledby="install-title" onClick={dismissTutorial}><div className="install-modal" onClick={e=>e.stopPropagation()}><button className="modal-close" aria-label="Fechar tutorial" onClick={dismissTutorial}>×</button><span className="eyebrow">No iPhone ou iPad</span><h2 id="install-title">Adicione à Tela de Início</h2><ol className="install-steps"><li><b>⇧</b><span>Toque no botão de compartilhar do navegador.</span></li><li><b>＋</b><span>Escolha “Adicionar à Tela de Início”.</span></li><li><b>✓</b><span>Toque em “Adicionar”.</span></li><li><b>🌿</b><span>Abra o Método LEVE pela tela inicial.</span></li></ol><button className="btn" onClick={dismissTutorial}>Entendi</button></div></div>}
 </>
}
